...
export const productSchema = joi.object({
    ...
    original_price: joi.number().min(1).required().messages(errorMessages("Giá gốc")),
    short_description: joi.string().required().messages(errorMessages("Mô tả ngắn")),
    description: joi.string().required().messages(errorMessages("Mô tả")),
    brand: joi.string().required().messages(errorMessages("Danh mục")),
    images: joi.array().min(1).required().items(joi.object({
    ...
    specifications: joi.array().optional().required().items(joi.object({
        _id: joi.string().optional(),
        name: joi.string().required().messages(errorMessages("Tên thông số")),
        attributes: joi.array().required().items(joi.object({
            _id: joi.string().optional(),
